...
<!-- Category Icon -->
<i class="fas fa-list" id="categoryIcon"></i>
...
card_button.onclick = function() {
   if (!isSpeaking) {
    ...
    if (/Chrome\/(\d+)/.test(navigator.userAgent)) {
      voice = voices.find((voice) => voice.name === 'Microsoft Zira - English (Canada)' && voice.lang === 'en-CA');
    }

   
    if (/Firefox[\/\s](\d+)/.test(navigator.userAgent)) {
      voice = voices.find((voice) => voice.name === 'Google US English' && voice.lang === 'en-CA');
    }

    
    if (/^((?!chrome|android).)*safari/i.test(navigator.userAgent)) {
      voice = voices.find((voice) => voice.name === 'Samantha' && voice.lang === 'en-CA');
    }

    if (!voice) {
     
      voice = voices.find((voice) => voice.name === 'Google US English' && voice.lang === 'en-US');
    }

    let rate = 0.9, pitch = 1, volume = 2;
    var ids =arrayColumn(years_all_info, 0);
    var id = ids.indexOf(cid);
    let text = years_all_info[id][7];

    speakData = new SpeechSynthesisUtterance(text);
