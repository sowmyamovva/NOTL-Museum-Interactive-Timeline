...
echo("<script>console.log(' in the save_data "."');</script>");
$config = new Config();
$connection = $config->getConnection();
// Check if form data is set
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    if(isset($_POST['id']) && isset($_POST['remove_event']))
    {
        $id = mysqli_real_escape_string($connection, $_POST['id']);
        $sql_sub_event_delete = "DELETE FROM sub_events WHERE event_id = '$id'";
        mysqli_query($connection, $sql_sub_event_delete);
        $sql_event_delete = "DELETE FROM events WHERE id = '$id'";
        mysqli_query($connection, $sql_event_delete);
    }
    else if (isset($_POST['id']) && isset($_POST['date']) && isset($_POST['event_title']) && isset($_POST['date_title']) && (isset($_POST['current_image_front']) || isset($_FILES['image_front'])) && (isset($_POST['current_image_back']) || isset($_FILES['image_back']))) 
    {
        ...
        if(isset($_POST['Add']))
        {
            ...
            $sql1 = "INSERT INTO events (date, date_title, event_title, category";
            $sql2 = "('$date','$date_title','$event_title','$category'";
        }
        else if ($_POST['Update'])
        {        
            $sql = "UPDATE events SET date = '$date', date_title = '$date_title', event_title = '$event_title', category='$category'";
        }
        if(isset($_POST['Add']))
        {
